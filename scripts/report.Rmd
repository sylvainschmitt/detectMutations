...
scheme <- normalizePath(snakemake@input[["scheme"]])
genomes_indexes <- snakemake@input[["index"]]
coverages_files <- snakemake@input[["coverage"]]
mutations_filtered <- vroom(snakemake@input[["mutations_filterd"]]) %>% 
...
```{r manual, include=FALSE, eval=F}
scheme <- "../dag/scheme.png"
genomes_indexes <- list.files("../results/reference/", pattern = "fa.fai", full.names = T)
coverages_files <- list.files("../results/alns/", pattern = ".regions.bed", full.names = T)
mutations_filtered <- vroom("../results/mutations/mutations_filtered.tsv") %>% 
  mutate(Filter = recode(Filter, "base" = "Base", "robust" = "EVS")) %>% 
  mutate(reference = recode(reference, "Fagus_sylvatica_mutant_v0" = "Mutant", 
                         "Fagus_sylvatica_revertant_v0" = "Revertant", "Fagus_sylvatica_v3" = "Ciron")) 
mutations <- vroom("../results/mutations.tsv")
genomes <- lapply(genomes_indexes, function(i)
       read_tsv(i, col_names = c("name", "length", "bytesindex", "basesperline", "bytesperline")) %>% 
         mutate(file = i)) %>% 
  bind_rows() %>% 
...
All analyses are currently done in the the [`hetre` branch of the `detectMutations` repository](https://github.com/sylvainschmitt/detectMutations/tree/hetre) (Fig. \@ref(fig:scheme)).

```{r scheme, fig.cap="Workflow for detecting mutations."}
include_graphics(scheme)
```
...
We found 970 to 1,272 mutations with base filtering across genomes, 
against  328 to 465 when using conservative filtering based on the empirical variant score (EVS) from *Strelka2* (Tab \@ref(tab:muttab)).
...
ggplot(mutations_filtered, aes(mutation_AF, col = Filter), fill = NA) +
...
ggplot(mutations, aes(mutation_type, fill = mutation_class)) +
  geom_bar() +
  xlab("Mutation type") +
  scale_fill_discrete("Mutation class")
```
...
ggplot(mutations, aes(mutation_spectra)) +
  geom_bar() +
...
The revertant sector shows an accumulation of $10^1$ to $10^3$ mutation from the mutant sector (Fig. \@ref(fig:rate)).

```{r rate, fig.cap="Cummulated number of mutations with increasing allelic frequency on the Revertant's genome."}
mutations %>% 
  ungroup() %>% 
  mutate(mutations = 1) %>% 
  group_by(mutation_af) %>% 
...
15k mutations are within transposable elements and 2.8k within genes (Fig. \@ref(fig:anottype)).
Among mutations within genes, most are in the mRNA, but 653 are in coding DNA sequence and 94 in untranslated regions (Fig. \@ref(fig:genetype)).
Among mutations within genes, a vast majority are within genes coding for non-specific serine/threonine protein kinase (Fig. \@ref(fig:go)).

```{r anottype, fig.cap="Type of annotations for all mutations."}
mutations %>% 
  mutate(type = "other") %>% 
  mutate(type = ifelse(!is.na(te_id), "TE", type)) %>% 
  mutate(type = ifelse(!is.na(gene_id), "gene", type)) %>% 
  group_by(type) %>% 
  summarise(N = n()) %>% 
...
```{r genetype, fig.cap="Type of gene sequence containing the mutations."}
mutations %>% 
  dplyr::select(gene_type) %>% 
  na.omit() %>% 
  mutate(type = "mRNA") %>% 
  mutate(type = ifelse(grepl("CDS", gene_type), "CDS", type)) %>% 
  mutate(type = ifelse(grepl("UTR", gene_type), "UTR", type)) %>% 
  group_by(type) %>% 
  summarise(N = n()) %>% 
  ggplot(aes(type, N, fill = type)) +
  geom_text(aes(label = N), vjust = 0) +
...
```{r go, fig.cap="Gene ontology for mutations within genes. Gene ontology is shown only for termes with more than 9 mutations."}
mutations %>% 
  dplyr::select(annotation_defline) %>% 
  na.omit() %>% 
  separate(annotation_defline, c("go_id", "go"), " - ") %>% 
  group_by(go) %>% 
  summarise(N = n()) %>% 
  arrange(desc(N)) %>% 
  ggplot(aes(reorder(go, N), N)) +
  geom_text(aes(label = N), hjust = 0) +
  geom_bar(stat = "identity") +
  coord_flip() +
  xlab("Gene ontology") + ylab("Count")
```
...
Final mutation dataset is available on [google drive](https://docs.google.com/spreadsheets/d/1gW43QUwcLurM-dAGIqJdjE_LTMOGBrdkvDTGSC5UvFM/edit?usp=sharing).
