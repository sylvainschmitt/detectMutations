...
```{r dp}
ggplot(filtered, aes(DP_lib, fill = (DP_lib > 50))) +
  geom_histogram() +
  theme_bw() +
...
```{r angela}
ggplot(filter(filtered, DP_lib > 50, tree == "angela"),
       aes(AF_lib, fill = tissue)) +
  geom_histogram() +
  theme_bw() +
  facet_wrap(~ gsub("Super-Scaffold_", "", SNV)) +
  theme(axis.text.x = element_text(angle = 90)) +
  ggtitle("Angela AF in fruits", "Depth above 50 reads") + xlab("") + 
  xlim(0,1)
```
```{r sixto}
ggplot(filter(filtered, DP_lib > 50, tree == "sixto"),
       aes(AF_lib, fill = tissue)) +
  geom_histogram() +
  theme_bw() +
  facet_wrap(~ gsub("Super-Scaffold_", "", SNV)) +
  theme(axis.text.x = element_text(angle = 90)) +
  ggtitle("Sixto AF in fruits", "Depth above 50 reads") + xlab("") + xlim(0,1)
```
