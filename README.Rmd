...
get_stats <- function(callfile, mutationsfile){
  mutations <- read_tsv(mutationsfile) %>% 
    mutate(True = 1) %>% 
    filter(REF != "N")
  call <- read.vcfR(callfile, verbose = F)@fix %>% 
    as.data.frame() %>% 
  ...
  stats <- full_join(mutations, call, by = c("CHROM", "POS", "REF", "ALT")) %>% 
    ...
    group_by(Confusion) %>% 
    summarise(N = n()) %>% 
    reshape2::dcast(. ~ Confusion) %>% 
    mutate(Precision = round(TP/(TP+FP), 2), Recall = round(TP/(TP+FN), 2))
  return(stats)
}
```
...
stats <- data.frame(libraries = list.files(config$libdir, pattern = "_R1.fastq")) %>% 
  mutate(libraries = gsub("_R1.fastq", "", libraries)) %>% 
  mutate(libraries = gsub("[[:alpha:]]", "", libraries)) %>% 
  separate(libraries, c("N", "R", "AF", "NR"), sep = "_", convert = T) %>% 
  mutate(caller = "mutect2") %>% 
  mutate(call = file.path("results", paste0("N", N, "_R", R, "_AF", AF, "_NR", NR),
                               caller, paste0("N", N, "_R", R, "_AF", AF, "_NR", NR, ".vcf"))) %>% 
  mutate(mutations = file.path(config$refdir, "..", "mutations", paste0(config$reference, "_mutated_N", N, "_R", R, ".tsv"))) %>% 
  group_by(N, R, AF, NR, caller) %>% 
  do(stats = get_stats(.$call, .$mutations)) %>% 
  unnest(stats) %>% 
  select(-`.`) %>% 
...
```{r statsTable}
kable(stats, col.names = c("Caller", "Number of mutations", "Transition Transversion Ratio", "Allele frequency", "Coverage",
                           "False Negative", "False Positive", "True Positive", "Precision (TP/TP+FP)", "Recall (TP/TP+FN)"),
      caption = "Caller assesment.")  
```
...
stats %>% 
  ...
  ggplot(aes(as.character(parameter.value), metric.value, col = caller, fill = caller)) +
