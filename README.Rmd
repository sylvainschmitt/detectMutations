...
get_stats <- function(callfile, mutationsfile){
  ...
  # callfile <- "results/N100_R2_AF0.1_NR2333/mutect2/N100_R2_AF0.1_NR2333.vcf"
...
stats <- data.frame(libraries = list.files(config$libdir, pattern = "_mutated_R1.fastq")) %>% 
  mutate(libraries = gsub("_mutated_R1.fastq", "", libraries)) %>% 
  mutate(libraries = gsub("[[:alpha:]]", "", libraries)) %>% 
  separate(libraries, c("N", "R", "AF", "NR"), sep = "_", convert = T) %>% 
  group_by(N, R, AF, NR) %>% 
  do(caller = c("mutect2", "freebayes", "gatk", "strelka2", "manta", "somaticsniper", "muse", "varscan")) %>% 
  unnest(caller) %>% 
  ungroup() %>% 
