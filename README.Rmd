...
all <- bind_rows(
  ...
  mutate(gatk = ifelse(GT %in% c("0|1", "0/1", "1/1", "1|1"), 1, 0))
rm(angela_vcf, sixto_vcf, angela, sixto)
```

```{r sixtofig, fig.height=7, fig.width=14, eval=FALSE}
gs <- all %>% 
  filter(tree == "sixto") %>%
...
```{r angelafig, fig.height=7, fig.width=14, eval=FALSE}
ga <- all %>% 
filter(tree == "angela") %>%
  bind_rows(all %>% 
              filter(tree == "angela") %>%
