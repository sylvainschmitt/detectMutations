...
```{r stats, eval=F}
refL <- width(readDNAStringSet("../generateMutations/data/Qrob_PM1N_1M.fa"))
data.frame(vcf = list.files("results/mutations", full.names = T)) %>% 
  ...
  write_tsv("stats.experiment2.tsv")
```

```{r }
read_tsv("stats.experiment1.tsv") %>% 
  filter(caller != "manta") %>% 
  filter(R == 2) %>% 
...
```{r}
dat <- read_tsv("stats.experiment1.tsv") %>%
  filter(caller != "manta") %>%
  mutate(caller = recode(caller,    
                         "mutect2" = "Mutect2", 
                         "freebayes" = "freebayes", 
...
* Singularity: https://github.com/sylvainschmitt/singularity-octopus/releases/download/0.0.1/sylvainschmitt-singularity-octopus.latest.sif
