...
```{r}
gc <- vroom::vroom("results/reference/Dgu_HS2_HYBRID_SCAFFOLD.gc", skip = 1,
             col_names = c("chrom", "start", "stop", "AT", "GC", "A", "C", "G", "T", "N", "other", "length")) %>% 
  mutate(pos = start + 500) %>% 
  mutate(chrom = gsub("Super-Scaffold_", "", chrom))
genome <- read_tsv("results/reference/Dgu_HS2_HYBRID_SCAFFOLD.fa.fai", 
...
```{r}
genome <- filter(genome, end > 2*10^6)
gc <- filter(gc, chrom %in% genome$name)
```

```{r, fig.height=10, fig.width=10}
circos.initializeWithIdeogram(as.data.frame(genome))
circos.genomicTrack(select(gc, chrom, start, stop, N), 
    panel.fun = function(region, value, ...) {
        circos.genomicLines(region, value, col = "blue", ...)
})

circos.genomicTrack(select(gc, chrom, start, stop, GC), 
    panel.fun = function(region, value, ...) {
        circos.genomicLines(region, value, col = "red", ...)
})
text(0, 0, "HS2", cex = 1.5)
legend("bottomleft", pch = 16, col = c("blue", "red"), legend = c("N", "GC"), bty = "n")
```
